...
test_y = test_df[output_col]


# normalizes data using Z-Score (just train)
...
# (USE_TUNER = False)
def build_model_regression_fixed(input_shape):
...
if USE_TUNER:
    ...
    for trial in all_trials:
        # for each trial, extract the score (ex: val_loss) and the hyperparameters
    ...
    print("--- MODO: Treinamento com Modelo Fixo para REGRESSÃO ---")
    model = build_model_regression_fixed([len(input_cols)])
...
# results
plt.scatter(test_y, test_predictions_scaled, marker='o', c='blue')
...
mae = mean_absolute_error(test_y, test_predictions_scaled)
rmse = np.sqrt(mean_squared_error(test_y, test_predictions_scaled))
correlation_coefficient = np.corrcoef(test_predictions_scaled, test_y.to_numpy())[0, 1]
...
# dataFrame de resultados usando o índice do conjunto de teste (test_df). loc para selecionar as linhas do DataFrame original 'data' que correspondem
# aos índices do nosso conjunto de teste. Isso nos dá as colunas originais ('stock', 'date') para as linhas corretas.
resultados_df = data.loc[test_df.index].copy()

# adds the column with the actual values (y_test). the index of y_test is already aligned with the results_df
resultados_df['valor_real'] = test_y

# Adicione a coluna com as previsões do modelo
# test_predictions é um array numpy, mas tem a mesma ordem e tamanho de y_test (índice deve ser o mesmo para uma junção segura)
resultados_df['valor_previsao'] = test_predictions_scaled

resultados_finais = resultados_df[['stock', 'date', 'valor_real', 'valor_previsao']]

print("\n--- Tabela de Comparação: Valores Reais vs Previsões ---")
print(resultados_finais.head(15).to_string())

# erro para cada previsão
resultados_finais['erro'] = resultados_finais['valor_previsao'] - resultados_finais['valor_real']
print("\n--- Análise de Erros (15 maiores erros) ---")
print(resultados_finais.reindex(resultados_finais.erro.abs().sort_values(ascending=False).index).head(15).to_string())



# makes predictions on normalized data and denormalizes
...
correlation_coefficient = np.corrcoef(train_predictions, train_y.to_numpy())[0, 1]
